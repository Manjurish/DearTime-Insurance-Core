...
class User extends Authenticatable implements Auditable
{
     ...
     */
    protected $hidden = [
        'password', 'remember_token', 'activation_token', 'created_at', 'updated_at', 'active',
    ];
...
//            return (($bankAccounts > 0) || ($bankCards > 0));
    ...
    public function newreferrer(){
        return $this->hasMany(User::class,'from_referrer');
    }
...
//        return 'profile_page';
    ...
    public function isCorporatePayer(){
        return $this->corporate_type == 'payorcorporate';
    }
   ...
   public function paymentsHistory(){
        ...
        $tt = Coverage::where('owner_id',$user->profile->id)->where('state','active')->get();
        ...
        $t =  CoverageOrder::whereIn('coverage_id',$ttdArray)->orderBy("id","desc")->first();
        $td = $t->order_id ?? null;

        $ttdd = Order::where('status','successful')->where('id',$td)->first();

        $transactions = Transaction::where('order_id', optional($ttdd)->id)->orWhereHas('order', function ($query) use ($user) {
    ...
    public function scopeAllUsersDTOrNonDT($q)
    {
        return $q->withoutGlobalScope(RegisteredScope::class);
