...
class ArekaController extends Controller
{
	...
	public function importCsv(Request $request)
	{
		...
		if(!empty($sheetData)){
			...
			foreach ($sheetData as $data) {
				...
				$input['premium_annually_death']      = $data[23];
				$input['premium_annually_accident']          = $data[24];
				$input['premium_annually_disability']      = $data[25];
				$input['full_premium_death']          = $data[26];
				$input['full_premium_accident']      = $data[27];
				$input['full_premium_disability']          = $data[28];
				$input['payment_without_loading_death']      = $data[29];
				$input['payment_without_loading_accident']      = $data[30];
				$input['payment_without_loading_disability']          = $data[31];
				$input['first_payment_on']      = $data[32];
				$input['next_payment_on']          = $data[33];
				$input['invoice_no']      = $data[34];
				$input['transaction_id']          = $data[35];
				$input['amount']      = $data[36];
				$input['payment_date']      = $data[37];
				$input['coverage_death']          = $data[20];
				$input['coverage_accident']      = $data[21];
				$input['coverage_disability']          = $data[22];
				$input['gateway']      = $data[38];
				$input['card_type']      = $data[39];
		...
		$testusers = VoucherDetails::whereIn('voucher_code',$upload_consider)->get();
	...
	// }
		...
		foreach($datas as $da){
			...
			foreach($covs as $cov){
				...
				$covv = Coverage::where('owner_id',$checkn->id)->where('payer_id',$payer_id)->where('product_name','Disability')->where('status','increase-unpaid')->first() ?? null;

				$covv_act = Coverage::where('owner_id',$checkn->id)->where('product_name','Disability')->where('status','active')->first() ?? null;
				...
				if($covv != null){
					...
					$covv->update(
								[
									...
									'renewal_date' =>$covv_act->renewal_date,
									'ndd_payment_due_date'=>$covv_act->next_payment_on,
									'payment_annually' =>$da->premium_annually_disability,
									'payment_monthly' =>Helpers::round_up($da->premium_annually_disability * 0.085, 2),
									'full_premium'  =>$da->premium_annually_disability,
									'payment_without_loading' => $da->premium_annually_disability,
									'has_loading' => 1,
									'payor_first_product_purchase_date' => $covv_act->first_payment_on,
									'payor_next_payment_date' => $covv_act->next_payment_on,
									'payor_last_payment_on' => $da->payment_date,
					...
					$covv_a = Coverage::where('owner_id',$checkn->id)->where('payer_id',$payer_id)->where('product_name','Disability')->where('status','unpaid')->first() ?? null;
				...
				$covv = Coverage::where('owner_id',$checkn->id)->where('payer_id',$payer_id)->where('product_name','Accident')->where('status','increase-unpaid')->first() ?? null;

				$covv_act = Coverage::where('owner_id',$checkn->id)->where('product_name','Accident')->where('status','active')->first() ?? null;
				...
				if($covv != null){
					...
					$covv->update(
								[
									...
									'renewal_date' =>$covv_act->renewal_date,
									'ndd_payment_due_date'=> $covv_act->next_payment_on,
									'payment_annually' =>$da->premium_annually_accident,
									'payment_monthly' =>Helpers::round_up($da->premium_annually_accident * 0.085, 2),
									'full_premium'  =>$da->premium_annually_accident,
									'payment_without_loading' => $da->premium_annually_accident,
									'has_loading' => 1,
									'payor_first_product_purchase_date' => $covv_act->first_payment_on,
									'payor_next_payment_date' => $covv_act->next_payment_on,
									'payor_last_payment_on' => $da->payment_date,
					 ...
					 $covv_a = Coverage::where('owner_id',$checkn->id)->where('payer_id',$payer_id)->where('product_name','Accident')->where('status','unpaid')->first() ?? null;
...
//  dd($sum_cov);
 ...
 $actions['new_death'] = 20000;
 $actions['new_accident'] = 20000;
 $actions['new_disability'] = 100000;
